...
class AppUsers
{
   ...
   */
    public function generateCode(){
      $data   = $this->_db->query("SELECT id FROM attendance_users ORDER BY id DESC LIMIT 1");
      $lastID = 0;
      if($data->count()):
	 ...
	 */
	 public function delete($id = null){
 		if(!$this->_db->delete('attendance_users',array('id', '=', $id))){
 			throw new Exception('There was a problem updating');
 		}
 	}
	...
	public function find($id = null, $outputField=null, $limit = null){
		if($id){
			...
			if(is_numeric($id)){
				$field = 'id';
				$data = $this->_db->getAll('attendance_users', array($field, '=', $id),$limit);
				if($data->count()){
