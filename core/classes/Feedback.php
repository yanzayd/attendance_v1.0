...
class Feedback
{
	...
	public function insert($fields = array()){
		if(!$this->_db->insert('healthplus_feedbacks', $fields)){
	...
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('healthplus_feedbacks',$id,$fields)){
	 ...
	 */
	 public function delete($id = null){
 		if(!$this->_db->delete('healthplus_feedbacks',array('id', '=', $id))){
	...
	public function find($id = null, $outputField=null, $limit = null){
		if($id){
			...
			if(is_numeric($id)){
				$field = 'id';
				$data = $this->_db->getAll('healthplus_feedbacks', array($field, '=', $id),$limit);
				if($data->count()){
	 ...
	 * @return Exception Message if errors
	 */
	public function findFeedback($id = null, $outputField=null, $limit = null){
		if($id){
			$hit = false;
			if(is_numeric($id)){
				$field = 'id';
	 ...
	 */
	 public function select($sql = null, $params=array()){
 		$data = $this->_db->query("SELECT* FROM healthplus_feedbacks {$sql}", $params);
 		if($data->count()){
