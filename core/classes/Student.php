...
class Student
{
   ...
   */
    public function generateCode(){
      $data   = $this->_db->query("SELECT id FROM attendance_student ORDER BY id DESC LIMIT 1");
      $lastID = 0;
      if($data->count()):
	...
	public function insert($fields = array()){
		if(!$this->_db->insert('attendance_student', $fields)){
	...
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('attendance_student',$id,$fields)){
	 ...
	 */
	 public function delete($id = null){
 		if(!$this->_db->delete('attendance_student',array('id', '=', $id))){
 			throw new Exception('There was a problem updating');
 		}
 	}
	...
	public function find($id = null, $outputField=null, $limit = null){
		if($id){
			...
			if(is_numeric($id)){
				$field = 'id';
				$data = $this->_db->getAll('attendance_student', array($field, '=', $id),$limit);
				if($data->count()){
	 ...
	 */
	 public function select($sql = null, $params=array()){
 		$data = $this->_db->query("SELECT* FROM attendance_student {$sql}", $params);
 		if($data->count()){
